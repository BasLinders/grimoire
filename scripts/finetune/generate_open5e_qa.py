...
Complements scripts/scrape/scrape_stackexchange.py's general-conversation data with
...
Usage
-----
    python scripts/finetune/generate_open5e_qa.py
    python scripts/finetune/generate_open5e_qa.py --types monsters --output data/finetune/open5e_monster_qa.jsonl
    python scripts/finetune/generate_open5e_qa.py --delay 0.3
    python scripts/finetune/generate_open5e_qa.py --document-slug ""  # every Open5e document, unfiltered

Then fine-tune the same way as the StackExchange-derived data (see
scripts/train/finetune_saga.py) -- concatenate this file with any other .jsonl
