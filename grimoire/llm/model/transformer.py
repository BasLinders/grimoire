    ...
    def forward(
        ...
        Args:
            input_ids: Integer tensor of shape ``(batch, seq_len)``
                containing token ids.  Values must be in
                ``[0, vocab_size)``.
            attention_mask: Optional tensor of shape ``(batch, seq_len)``
                with ``1`` for real tokens and ``0`` for padding.  When
                provided, padded positions receive ``-inf`` attention scores
                so they do not influence any other token's representation.

        Returns:
            Float tensor of shape ``(batch, seq_len, vocab_size)``
            containing unnormalised log-probability (logit) distributions.
            Apply ``softmax`` along the last dimension to get probabilities,
            or use ``cross_entropy`` directly on logits during training.
        """
