...
| **Training UI** | Gradio app: Preprocess, Pre-train (size presets, gradient checkpointing, Chinchilla-optimal step suggestion from corpus size), Fine-tune (LoRA rank/alpha/targets, step suggestion from dataset example count), Scale (Chinchilla calculator), Evaluate, Ingest, Corpus (pre-build semantic index), Chat (int8 toggle, adaptive temperature, retrieval controls, math tool) tabs | ✓ done |
